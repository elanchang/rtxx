...
int writev(srs_hijack_io_t ctx,int fd, struct iovec *iov, int iovcnt)
{
    ssize_t nwrite = 0;
    int i=0;
    int error = 0;
    ...
    for (i = 0; i < iovcnt; i++) {
        ...
        int len;
        int nsent;
        size_t current_len=current->iov_len;
        len=current_len;
   again_write:             
        nsent=send(fd, (char*)current->iov_base, len, 0);
        ...
        nwrite += current_len;
        if (nsent == 0) {
            print_msg("close socket by peer\n");
            return nwrite;
        }
    }
    return nwrite;
...
int srs_hijack_io_writev(srs_hijack_io_t ctx, struct iovec *iov, int iov_size, size_t* nwrite){
    ...
    struct SrsBlockSyncSocket* skt = (struct SrsBlockSyncSocket*)ctx;
    int ret = ERROR_SUCCESS;
    int nb_write = writev(ctx,skt->fd, iov, iov_size);
    if (nwrite) {
        *nwrite = nb_write;
    } 
